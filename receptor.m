...
mensagem_inicial = zeros(1, 100);             % Pré-aloca com tamanho suficiente
...
while(1)                       % Espera para ler a mensagem inicial
    ...
    mensagem_inicial(i) = tmp;                % Armazena o byte
    if (mensagem_inicial(i) == 10)            % Se for lido um enter (10 em ASCII)
        break;                 % Sai do loop
    endif
    i = i + 1;                 % Incrementa o índice de leitura
end

mensagem_inicial = mensagem_inicial(1:i);                    % Ajusta o tamanho final do array
c = char(mensagem_inicial);                   % Transformando caracteres recebidos em string
...
try
    while (1)                   % Loop infinito (até Ctrl+C)
        ...
        if (length(data) > 0)
            dados_digitais = data;                  % Armazena os dados brutos

            % Acumula dados para melhor análise de erros (mantém até MAX_RESULTS*5)
            data_history = [data_history, dados_digitais];
            if (length(data_history) > MAX_RESULTS*5)
                data_history = data_history(end-MAX_RESULTS*5+1:end);
            endif

            time = (0:length(dados_digitais)-1)/fs; % Vetor de tempo normalizado (em segundos)

            % Atualiza o primeiro subplot - Sinal Reconstruído
            subplot(h1);
            plot(time, dados_digitais*5/1023);      % Converte os valores ADC para tensão (0-5V)
            xlabel('t(s)');
            ylabel('Tensão (V)');
            ...
            subplot(h2);
            stem(dados_digitais,'.');                   % Plota amostras discretas
            xlabel('n');
            ylabel('Valor ADC');
            title('x[n]');
            grid on;
            ...
            subplot(h3);
            stairs(dados_digitais);                 % Plota amostras regulares em forma de escada
            xlabel('n');
            ylabel('Valor ADC');
            title('x[n] segurado');
            grid on;
